...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lf = np.zeros(shape=(9, 9, 480, 640, 3), dtype=int)

        """ Read inputs """
        for i in range(9):
            for j in range(9):
                file_path = os.path.join(self.trainset_dir, scene_name, f'{i+1}_{j+1}.png')
                temp = imageio.imread(file_path)
                lf[i, j, :, :, :] = temp

        dispGT = np.load(self.trainset_dir + scene_name + '/5_5_disparity.npy')
        ...
        while (sum_diff < 0.01):
            ...
            sum_diff = np.sum(np.abs(lf_crop[self.angRes // 2, self.angRes // 2, :, :] -
                                         np.squeeze(lf_crop[
                                                        self.angRes // 2, self.angRes // 2, self.patchsize // 2, self.patchsize // 2]))
...
def refocus_augmentation(lf, dispGT):
    ...
    if refocus_rand == 0:
        ...
        center = (angRes - 1) // 2
        min_d = int(np.min(dispGT[:, :]))
        max_d = int(np.max(dispGT[:, :]))
        dispLen = 6 - (max_d - min_d)
        k = np.random.randint(dispLen + 1) - 3
        dd = k - min_d
        out_dispGT = np.zeros((h, w), dtype=float)
